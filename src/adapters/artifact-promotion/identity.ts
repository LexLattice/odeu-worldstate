export interface ArtifactPromotionIdentityMaterial {
...
/** A small synchronous SHA-256 for browser-safe artifact IDs and fingerprints. */
export function artifactIdentitySha256Hex(value: string): string {
...
export function artifactPromotionId(
  input: ArtifactPromotionIdentityMaterial,
): string {
  return `artifact-promotion:sha256:${artifactIdentitySha256Hex(
    canonicalJson(input),
  )}`;
}
